...
## Mapping the vector index for West Nile Virus in the Sacramento Metropolitan Area, United States

In this section, we use entomological surveillance data from the `vectorsurvR` R package, which provides records of mosquito collections, species identification, trapping effort, and pathogen testing across multiple locations in the United States. The dataset includes pool-level information such as counts, species, trap dates and locations, and laboratory test results for arboviruses.

This analysis focuses on *Culex pipiens* in the Sacramento Metropolitan Area (SMA), California, from 2015 to 2021. *Cx. pipiens* is one of the primary vectors of West Nile virus (WNV) in North America [@petersen2013wnv; @kramer2007wnv].

WNV is a mosquito borne flavivirus sustained in an enzootic cycle, that is, persistent circulation of the virus within non human animal populations, primarily birds, in a defined geographic area. Transmission is maintained between *Culex* mosquitoes and birds, while humans and other mammals are incidental dead end hosts that do not contribute to further transmission. Humans and other mammals are incidental, dead-end hosts. Most human infections are asymptomatic, a minority present with febrile illness, and a small proportion develop neuroinvasive disease [@petersen2013wnv]. Surveillance of both vector abundance and WNV infection prevalence is therefore important for anticipating human risk and guiding public health interventions.

In this analysis, we quantify local entomological risk using the *vector index (VI)*, defined as the product of the average number of female mosquitoes per trap-night and the proportion of mosquitoes testing positive for the pathogen: $$
\mathrm{VI} = \text{abundance} \times \text{proportion positive}.
$$
where *abundance* is the mean number of female mosquitoes per trap-night and *proportion positive* is the proportion of tested pools that are positive. In practice, infection prevalence is estimated from PCR-tested mosquito pools using maximum likelihood estimators for pooled samples [@biggerstaff2006pooledinf; @cdc_wnv_guidelines_2024]; many operational analyses also report the minimum infection rate and apply these estimates when computing the vector index [e.g., @bolling2009vi; @jones2011vi].

In what follows, we aggregate collections by site and epidemiologic week, estimate species-specific infection prevalence from pooled testing results, and map the resulting VI for *Cx. pipiens* across the SMA. This provides a spatial summary of where and when the abundance of infected vectors is highest, which can support targeting of vector control and public health messaging [@bolling2009vi].

### Modelling the abundance of *Culex pipiens*

We begin by loading the necessary libraries and the data derived from the `vectorsurvR` package in R. The original datasets, `sample_pools` and `sample_collections`, contain mosquito surveillance data from California, including species identification, collection date, location, pathogen testing results, trap type, and trapping effort. For this example, these have been processed to produce `abund_sma`, which summarises the abundance of female *Culex pipiens* within the Sacramento Metropolitan Area. 
