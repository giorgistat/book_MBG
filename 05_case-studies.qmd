...
simulate_random_effects <- function(model, nsim = 1000, grid_range = c(-10, 10), grid_length = 5000) {
  ...
  # Log-posterior density function for one group
  log_posterior_u <- function(u, y_j, X_j, beta_hat, sigma2_hat) {
  ...
  # For each group, compute posterior samples of u_j
  for (j in seq_along(unique_groups)) {
    ...
    # Create grid and compute log posterior
    u_grid <- seq(grid_range[1], grid_range[2], length.out = grid_length)
    logdens <- sapply(u_grid, log_posterior_u, y_j = y_j, X_j = X_j,
...
## Mapping the entomological incoluation rate for West Nile Virus in Placer County, United States
...
This analysis focuses on the mosquito species *Culex pipiens* in Placer County, California, over the period 2015 to 2021. *Cx. pipiens* is one of the primary vector species responsible for the transmission of West Nile Virus (WNV) in North America. This time-aggregated subset allows us to investigate spatial patterns of mosquito abundance and WNV presence over several years of surveillance.
...
```
To provide spatial context, we use the `rgeoboundaries` package to download second-level administrative boundaries (ADM2) for the United States. We extract counties from California and subset to those overlapping with our trap locations. In this case, the relevant region includes Sacramento, Placer, and El Dorado counties — counties that collectively fall within the broader Sacramento Metropolitan Area (SMA).
...
Visual inspection of the top panel in @fig-wnv-years suggests that, although in some years the surveys were carried out in different areas, there is still some spatial overlap that may help us to estimate the year-to-year variation. This observation indicates that spatial coverage was not uniform over time, and motivates caution when interpreting temporal trends in abundance or infection risk.

Nonetheless, the bottom panel of @fig-wnv-years shows that the average number of mosquitoes captured per trap was relatively stable across years, with the exception of noticeably lower values from 2019 to 2021. This suggests that overall mosquito activity did not vary drastically, at least when aggregated annually. Moreover, the presence of a reasonable degree of spatial overlap between years supports the inclusion of survey year as a covariate in geostatistical models, to account for temporal effects while retaining sufficient spatial information for reliable inference.
...
### Modelling number of mosquitoes
