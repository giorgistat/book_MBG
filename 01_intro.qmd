...
| `liberia`             | Prevalence data on river-blindness from Libera                                        | Example              |
...
### The Matern family of covariance function

Throughout the book, we shall consider the @matern2013 family of covariance functions to model the spatial correlation of the Gaussian process $S(x)$. This defined as $$
...
#| fig-cap: "Plots of three satationary and isotropic correlation functions of the Matern family, with shape and scale parameters as indicated in the legend. The scale of the spatial correlation is choses so that each of the three functions reaches 0.05 at distance 0.3 (as also shown by the dahsed black segments). "
...
matplot(u, cbind(m0.5,m1.5,m2.5), type="l",
        ...
        main = "Matern correlation functions example")
...
#| fig-cap: "Simulates spatial surface using the three correltion functions shown in @fig-matern-func: (a) $\\phi = 0.1$ and $\\kappa=0.5$; (b) $\\phi=0.063$ and $\\kappa=1.5$; (c) $\\phi=0.051$ and $\\kappa=2.5$."
