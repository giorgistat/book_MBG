...
```{r echo=FALSE, message=FALSE}
#| label: fig-italy-sim-ch1
#| fig-cap: "Map of the locations of the simulated data-set generated over Italy for a continuous outcome."

library(sf)
library(mapview)
library(RiskMap)
data(italy_sim)
italy_sim_sf <- st_as_sf(italy_sim, coords = c("x1", "x2"), 
                       crs = 32634)
map <- mapview(italy_sim_sf, zcol = "y", layer.name = "Outcome") 
mapview::mapshot(map, file = "./figures/italy_sim_ch1.png")
...
### The Matern family of correlation functions {#sec-metern-correlation}

Throughout the book, we shall consider the @matern2013 family of correlation functions to model the spatial correlation of the Gaussian process $S(x)$. This defined as $$
...
par(mfrow=c(1,2))
matplot(u, cbind(m1,m2,m3), type="l",
        lty=c("solid", "dotted", "dashed"), col=1:3, lwd=2,
        ylab="Spatial correlation", xlab="Distance between locations",
        main = "(a)")


legend(0.2,0.8,c(expression(varphi~"="~0.05 ~", " ~ kappa ~"= 0.5"),
                 expression(varphi~"="~0.1~", " ~ kappa ~"= 0.5"),
                 expression(varphi~"="~0.2~", " ~ kappa ~"= 0.5")),
       lty=c("solid", "dotted", "dashed"), col=1:3, lwd=2) 
...
matplot(u, cbind(m0.5,m1.5,m2.5), type="l",
        lty=c("solid", "dotted", "dashed"), col=1:3, lwd=2,
        ylab="Spatial correlation", xlab="Distance between locations",
        main = "(b)")
segments(0,0.05,0.3,0.05,lty="dashed")
segments(0.3,0,0.3,0.05,lty="dashed")

legend(0.2,0.8,c(expression(varphi~"="~0.100 ~", " ~ kappa ~"= 0.5"),
                 expression(varphi~"="~0.063~", " ~ kappa ~"= 1.5"),
                 expression(varphi~"="~0.051~", " ~ kappa ~"= 2.5")),
       lty=c("solid", "dotted", "dashed"), col=1:3, lwd=2) 
```
...
The flexibility provided by the Matern correlation function in capturing different forms of spatial correlations has made one of, if not the most widely used correlation function in model-based geostatistics [@stein1999]. For this reason, in this book we will consider the Matern correlation function. We will consider estimation issues related to the Matern correlation in @sec-estimation.
