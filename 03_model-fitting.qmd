...
### Maximum likelihood estimation for non-Guassian generalized linear geostatistical models
...
MCML solves this problem using Monte Carlo methods, which involve random sampling to approximate the value of these integrals.
One way to apply Monte Carlo methods in this context is through *importance sampling*. Importance sampling allows us to approximate the likelihood function by rewriting the likelihood in a form that can be estimated using samples from an easier distribution, known as the *importance sampling distribution*. To thise end, we then rewrite @eq-glgm-lik2 as
$$
\begin{align}
L(\theta) 
&= \int [W, Y; \theta] \: dW  \\
&= \int \frac{[W, Y; \theta]}{[S, Y; \theta_0]}[W, Y; \theta_0]\: dW   \\
...
2.    Simulate Samples from $[W \mid Y; \theta_0]$ using MCMC methods.
...
In `RiskMap` steps 1 to 3 are automated, whilst 4 to 7 need to be manually implemented.
...
7.  Consider a linear model for the Galicia data as in @sec-glm-parest but now also introduce a nugget term, hence \$\$ Y_i = \mu + S(x_i) + Z_i + U_i
