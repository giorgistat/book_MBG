...
In section @sec-agg-count-exp, we carried out the exploratory analysis of the riverblindness prevalence data to assess how the covariate of elevation could be introduced into the model. In @sec-empirical-variog-bin, we then tested for residual spatial correlation, when the log-transformed elevation is used as a covariate, using the empirical variogram. Based on these results, we now formulate a fit a Binomial geostatistical model to data. To achieve this, we extend the model presented in @eq-liberia-bin-mixed-elev through the inclusion of a spatial Gaussian process, $S(x)$, hence we write $$
...
We use the Monte Carlo maximum likelihood (MCML) method to carry out the parameter estimation (see @sec-mcml), which is implemented in the `glgpm` function.
...
### The likelihood function of a non-spatial generalized linear mixed model
...
### The likelihood function of a genearlized linear geostatistical model
...
$$ {#eq-glgm-lik1} To simplify the explanation and notation in this section and the following ones, we will first rewrite the likelihood function in terms of the random effect ( W_i = S(x_i) + Z_i ) and will denote this in its vector form as $W = (W_1, \ldots, W_n)$. Hence we write 
$$L(\theta) = [Y] = \int_{R^n} \: [W] [Y \:|\: W] \: d W$$ {#eq-glgm-lik2} 
...
In the next two sections we provide a brief overview of Markov chain Monte Carlo and how these can be used to approximate the likelihood function in @eq-glgm-lik2.

#### Monte Carlo Markov Chain: sampling from the predictive distribution of the spatial Gaussian process

Let us consider the problem of sampling from the distribution of $[W | Y]$, where $W$ and $Y$ are the random variables with the same properties as previously introduced. Although the distribution $[W | Y]$ has not yet appeared in the derivation of the likelihood function of a generalized linear geostatistical model, sampling from $[W | Y]$ will be a crucial step in our approach to maximum likelihood estimation which we will illustrate in the next sub-section. However, direct simulation from this distribution is not feasible, hence we have to resort to Markov Chain Monte Carlo (MCMC) methods which we now briefly introduce.
...
where $\epsilon$ is a step size parameter, $\nabla \log \pi(W_{(k)})$ is the gradient of the log of the target density with respect to $W_{(k)}$, and $\eta \sim \mathcal{N}(0, I)$ is a Gaussian random variable with mean 0 and indendent components with unit variance.
...
$$
where $N$ is the total number of MCMC samples and $\rho_k$ is the autocorrelation at lag $k$. A high ESS indicates that the chain has good mixing properties and the samples are nearly independent. The summation of autocorrelations up to a sufficiently large lag is used to estimate the reduction in effective sample size due to the dependence between samples. A small ESS suggests high autocorrelation and that more samples are needed to achieve the same precision as a set of independent samples. The ESS calculation is detailed in @geyer1992practical.

In addition to those, other diagnostics can provide further assurance of convergence. For example, the Gelman-Rubin diagnostic compares the variance between multiple chains to the variance within each chain; a diagnostic value close to 1 indicates that the chains have converged to a common distribution [@gelman1992inference]. Also, the Geweke diagnostic tests for convergence compares the means of the first part and last part of a single MCMC chain. If the means are not significantly different, it suggests that the chain has reached stationarity [@geweke1992evaluating]. Employing these additional diagnostics, along with those listed above, provides a comprehensive approach to assessing the convergence and reliability of MCMC results. Using a combination of these diagnostics helps to robustly assess whether the MCMC algorithm has adequately explored the target distribution.

For a general overview of MCMC algorithms, we refer you to @brooks2011handbook and @geyer2019handbook.
...
4.  Create an R function that computes the *cloud variogram*. As explained in @sec-expl-spatial, the cloud variogram is obtained by plotting $\hat{V}_{ij}$ (see @eq-hat-squared-diff) against the distances $u_{ij}$. The function should take as input a data-set with three columns: the variable for which the cloud variogram is to be computed; and two columns corresponding to the location of the data. Then, use this function to create the cloud variogram for the model for river-blindness in @eq-liberia-bin-mixed-elev. How does this compare to empirical variogram that takes the averages within predefined distance classes, as shown in @fig-liberia-variog?
...
7.  Consider a linear model for the Galicia data as in @sec-glm-parest but now also introduce a nugget term, hence $$
