...
We now illustrate how fit a generalize linear mixed, using the `anopheles` data-set as an example. We then consider two models, an intercept-only model and one that uses elevation as a covariate. Let $\mu(x_i)$ be the number of mosquitoes captured at a location $x_i$; then the linear predictor with elevation as a covariate takes the form
$$
...
We carry out the estimation in R using the `glmer` function from the `lme4` package (see @bates2015 for a detailed tutorial). The `glmer` function implements the maximum likelihood estimation of generalized linear mixed models 
...
fit_glmer_elev_int <- glmer(An.gambiae ~ (1|ID_loc), family = poisson, 
                        data = anopheles, nAGQ = 25)
```
