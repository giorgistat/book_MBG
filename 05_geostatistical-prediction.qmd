...
```
In the code above, we have set `view_all = FALSE` to display only the overall average. If `view_all = TRUE`, the scores are instead reported for each test set, in addition to the overall average. 
The results from the `summary` suggest that the two models have a similar predictive performance in terms of both CRPS and SCRPS, regardless of the sampling scheme used to create the test sets. To examine this further we can plot a point map of the CRPS and SCRPS using the locations of the test sets. 
...
### Step 1: Define the aim of the simulation study
...
#### Example
In our analysis of the riverblindness data-set, we might be interested in assessing the usefulness of elevation as a spatial predictor. To this end, we can use a simulation study where we compare the performance of models that make use of elevation against models that do not use it for prediction of disease prevalence. 

In this case, we can consider the following true model (henceforth $M_T$) for the simulation
$$M_{T}: \:
\log\left\{\frac{p(x_i)}{1-p(x_i)}\right\} = \beta_{0} + \beta_{1}e(x_i) + \beta_{2}\max\{e(x_i)-150, 0\} + S(x_i).
$$ {#eq-true-model-lib}
Hence, the true model expressed by the equation above assumes a non-linear relationship with elevation based on a linear spline that has a single change point in 150 meters, which we have previously considered in our analysis. In order to simulation from the the true model, which is the next step of the simulation study, we first need to define the values of the model parameters. A reasonable choice is to set the values to the maximum likelihood estimates obtained by fitting the binomial geostatistical model in @eq-true-model-lib. These can be obtained as shown below. 
```{r, eval=FALSE}
set.seed(123)
...
coef(true_model)
```
A natural candidate model to address the usefulness of using elevation as a covariate a an intercept only model (henceforth $M_C$), i.e.
$$M_{C}: \:
\log\left\{\frac{p(x_i)}{1-p(x_i)}\right\} = \beta_{0} + S(x_i).
$$ {#eq-true-model-lib}
This model does not make use of any covariates and can be usually selected as a candidate model whenever the goal is to assess the usefulness of covariates for spatial prediction. 
...
### Step 3: Fit the candidate models to the simulated data and predict the target
...
$$
where $e(x)$ is the elevation in meters at location $x$ and $S(x_i)$ is a stationary and isotropic Gaussian process with variance $\sigma^2$ and exponential correlation function $\rho(u) = \exp\{-u/\phi\}, \phi>0$.
Consider also an intercept only model
$$
