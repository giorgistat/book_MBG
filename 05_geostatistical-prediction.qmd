...
### Assessing calibration using graphical tools
...
In our example on riverblindness mapping, we now consider two models. 
A mode, which we call $M_{0}$, which is an intercept only geostatistical model, with linear predictor
$$
...
```
```{r, collapse=TRUE, fig.height=8, width=16}
#| label: fig-anpit-reg
...
```
```{r, collapse=TRUE, fig.height=8, width=14}
#| label: fig-anpit-clus
#| fig-cap: "The average nonrandomized probability integral transform (AnPIT) averaged across 10 test sets of size 9 (upper panel) and for each of the 10 tests sets (lower panel). The test sets are generated using a clustering algorithm based on the locations of the data."
...
### Assessing calibration and sharpness using metrics of predictive performance
